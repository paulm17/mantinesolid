...
export function createStorage<T>(type: StorageType, hookName: string) {
  ...
  return function useStorage({
    ...
    const readStorageValue = createMemo(() => {
      return (skipStorage?: boolean): T => {
    ...
    const setStorageValue = createMemo(() => {
      return (val: T | ((prevState: T) => T)) => {
    ...
    const removeStorageValue = createMemo(() => {
      return () => {
        removeItem(key);
