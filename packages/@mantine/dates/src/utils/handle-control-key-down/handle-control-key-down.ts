type ControlsRef = HTMLButtonElement[][][];
...
function focusOnNextFocusableControl({
  ...
  const controlToFocus =
    controlsRef?.[nextIndex.levelIndex]?.[nextIndex.rowIndex]?.[nextIndex.cellIndex];
...
function getControlsSize(controlsRef: ControlsRef) {
  return controlsRef?.map((column) => column.map((row) => row.length));
}
