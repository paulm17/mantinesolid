...
export const Timeline = factory<TimelineFactory>((_props, ref) => {
  ...
  const items = _children.toArray()?.map((item: any, index: number) =>
    cloneElement(item, {
      unstyled: local.unstyled,
      __align: local.align,
      __active:
        item.props?.active ||
        (local.reverseActive ? local.active! >= _children.length - index - 1 : local.active! >= index),
      __lineActive:
        item.props?.lineActive ||
        (local.reverseActive ? local.active! >= _children.length - index - 1 : local.active! - 1 >= index),
    })
  );
