...
import { clamp, PossibleRef, useMergedRef, useMove, useUncontrolled } from '@mantine/hooks';
import {
  BoxProps,
  createVarsResolver,
  ElementProps,
...
export const Slider = factory<SliderFactory>((_props, ref) => {
  ...
  let valueRef = _value();
  let onChangeEndRef = local.onChangeEnd ;
  ...
  createEffect(() => {
    onChangeEndRef = local.onChangeEnd;
  });

  const root = null as HTMLDivElement | null;
  const thumb = null as HTMLDivElement | null;
  const position = getPosition({ value: _value(), min: local.min!, max: local.max! });
  ...
  const mergedRef = useMergedRef(root as PossibleRef<HTMLDivElement>, (el) => {
    if (el) {
      ref;
    }
  });
