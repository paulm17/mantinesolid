import { createSignal, createEffect, onMount, onCleanup, JSX, For, splitProps } from "solid-js";
...
export const PinInput = factory<PinInputFactory>((_props, ref) => {
  ...
  const [_value, setValues] = useUncontrolled<string[]>({
    ...
    defaultValue: local.defaultValue?.split('').slice(0, length ?? 0),
  ...
  createSignal(() => {
    if (local.value === '') {
      setValues(createPinArray(length ?? 0, local.value));
    }
  });
        ...
        <For each={_value()}>
          {(char, index) => (
            <Input
              ...
              ref={mergeRefs(ref, el => inputsRef[index()] = el!)}
